/**
  * Universidad de La Laguna
  * Escuela Superior de Ingeniería y Tecnología
  * Grado en Ingeniería Informática
  * Algoritmos y Estructura de Datos 2021-2022
  *
  * @file rational_t.cpp
  * @author Eric Angueta Rogel alu0101335339@ull.edu.es
  * @date 08/03/2022
  * @brief Este programa realiza operaciones con numeros racionales.
  * @bug There are no known bugs
  * @see se indican entre [] las pautas de estilo aplicadas de
  *        "C++ Programming Style Guidelines"
  *        https://geosoft.no/development/cppstyle.html
  */
// pauta de estilo [92]: comentarios multilínea usando solo "//"

#include "rational_t.hpp"
#include <cmath>

rational_t::rational_t(const int n, const int d)
{
  assert(d != 0);
  num_ = n, den_ = d;
}

// pauta de estilo [87]: 3 líneas de separación entre métodos

// pauta de estilo [83]: tipo retornado en línea anterior al método
int
rational_t::get_num() const
{
  return num_;
}



int
rational_t::get_den() const
{
  return den_;
}


  
void
rational_t::set_num(const int n)
{
  num_ = n;
}


  
void
rational_t::set_den(const int d)
{
  assert(d != 0);
  den_ = d;
}



double
rational_t::value() const
{ 
  return double(get_num()) / get_den();
}


// comparaciones
bool
rational_t::is_equal(const rational_t& r, const double precision) const //por que se utiliza el const al final de la expresion
{
  bool comparacion;
  fabs(value() - r.value()) < precision ? comparacion = true : comparacion = false;
  
  return comparacion;   
}



bool
rational_t::is_greater(const rational_t& r, const double precision) const
{
  bool comparacion;
  value() - r.value() > precision ? comparacion = false : comparacion = true;

  return comparacion;
}



bool
rational_t::is_less(const rational_t& r, const double precision) const
{
  bool comparacion;
  r.value() - value() > precision ? comparacion = false : comparacion = true;

  return comparacion;
}


/**
 * @brief   
 * 
 * @param r 
 * @return rational_t 
 */
rational_t
rational_t::add(const rational_t& r)
{
  int num, den;
  if (get_den() != r.get_den()) 
  {

    den = get_den() * r.get_den();
    num = (get_den() * r.get_num()) + (r.get_den() * get_num());

  } else {

    num = get_num() + r.get_num();
  
  }
  rational_t result(num, den);

  return result;
}



rational_t
rational_t::substract(const rational_t& r)
{
  int num, den;
  if (get_den() != r.get_den()) 
  {

    den = get_den() * r.get_den();
    num = (get_den() * r.get_num()) - (r.get_den() * get_num());
  
  } else {

    num = get_num() - r.get_num();
  
  }
  rational_t result(num, den);

  return result;
}



rational_t
rational_t::multiply(const rational_t& r)
{
  int num, den;
  den = get_den() * r.get_den();
  num = get_num() * r.get_num();

  rational_t result(num, den);

  return result;
}



rational_t
rational_t::divide(const rational_t& r)
{
  int num, den;
  den = get_den() * r.get_num();
  num = get_num() * r.get_den();

  rational_t result(num, den);

  return result;
}



// E/S
void
rational_t::write(ostream& os) const
{
  os << get_num() << "/" << get_den() << " = " << value() << endl;
}



void 
rational_t::read(istream& is)
{
  cout << "Numerador? ";
  is >> num_;
  cout << "Denominador? ";
  is >> den_;
  assert(den_ != 0);
}

//MODIFICACION
rational_t
rational_t::opposite(void) const
{
  rational_t result(-1 * get_num(), get_den());

  return result;
}
